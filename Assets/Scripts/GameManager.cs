...
public class GameManager : MonoBehaviour
{
    ...
    float cntdnw = 120.0f, xTimer = 3.0F, roundTimer = 5.0F;
    GameObject timer;
    TMPro.TextMeshProUGUI timerText;
    bool isPlayTime, flag, xActive, nextRound;
    int currentRound;
        ...
        if (Input.GetKeyDown(KeyCode.Return))
        {
            if (lettersTyped == 5)
            {
                ...
                } else
                {
                    ...
                    XText.alpha = 1;
                    xTimer = 3.0F;
                    X.transform.position = new Vector2(X.transform.position.x, 350 - 110 * (wordsEntered) + 540);
                    xActive = true;
                }
            }
        }
